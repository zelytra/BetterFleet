...
/// frontend's "a VPN / ping optimizer intercepts the game's traffic" hint. Counts cycles where the
...
/// interceptor signature as an empty enumeration EVERY cycle for many minutes, which is what a
...
pub async fn init() -> std::result::Result<Arc<RwLock<Api>>, anyhow::Error> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            {
                // Interceptor signal (#801): count consecutive empty enumerations while the game
...
mod tests {
    ...
    // The interceptor signal (#801): the counter must ride through the whole "game process alive,
    ...
    fn empty_enumerations_accumulate_only_while_the_game_runs() {
        // A blind stretch: the count rises cycle after cycle - the VPN-interceptor signature.
        let mut cycles = 0;
        for _ in 0..3 {
        ...
        // Saturating, not wrapping: an interceptor left on for days must not reset the signal.
