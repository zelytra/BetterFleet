...
/**
 * The website's locale files, guarded the way the webapp's have been since #603 - it had no parity
 * test at all until #859, so a key missing from any of its fourteen files rendered as its own path
 * ("seo.statistics.title" sitting where a page title belongs) with nothing failing.
 *
 * Reads the sources rather than trusting them: every t("...") the site uses must resolve in every
 * locale.
 */
